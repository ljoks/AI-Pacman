...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for action in actions:
          if bestValue <= self.getQValue(state, action) or bestValue == float("-inf"):
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        diff = (reward + (self.getValue(nextState) * self.discount)) - self.getQValue(state, action)
        feats = self.featExtractor.getFeatures(state, action)
        for feat in feats:
          self.weights[feat] = self.weights[feat] + (self.alpha * feats[feat] * diff)
